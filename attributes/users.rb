#
# Cookbook Name:: base2
# Attribute:: users
#
# Copyright (C) 2014 base2Services
#
# All rights reserved - Do Not Redistribute
#

default['base2']['users']['base2'] = {
    'ssh_keys' => [
        'ssh-rsa  lshobbrook',
        'ssh-rsa  jbrook',
        'ssh-dss  awalker',
        'ssh-rsa  dmartin',
        'ssh-rsa  npajtic',
        'ssh-rsa  avine',
        'ssh-rsa  cdelcourt',
        'ssh-rsa  rmanto',
        'ssh-rsa  n.clark@base2services.com',
        'ssh-rsa  zye',
    ],
    'sudo' => true
}


default['base2']['users']['ciinabox-metrics'] = {
    'ssh_keys' => [],
    'sudo' => false
}

# this attribute is an array of hashes and is overriden in a wrapper cookbook e.g
# node.override['base2']['windows']['users'] = [
#   {
#     username: 'base2'
#     password: 'supersecret'
#     groups: ['Administrators']
#   }
# ]
default['base2']['windows']['users'] = []
