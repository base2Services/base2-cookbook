Param(
  [string]$RuntimeCookbook = "runtime",
...
function Invoke-Chef {
  Param(
    ...
    [string][Parameter(Mandatory=$True)]$CookbookName,
    [string][Parameter(Mandatory=$True)]$Role
  )

  $chef_args = "--local-mode -o recipe['$CookbookName::$role']"
...
if($environment -and $role) {
  Write-Output "Invoking chef for $environment environment, $role role"
	Invoke-Chef -Environment $environment -EnvironmentType $environmenttype -CookbookName $RuntimeCookbook -Role $role
} else {
